/**
 * Čisti formateri dijagnostičkih izveštaja (bez bočnih efekata): DTO → string.
 * Vraćaju .txt / .md / PDF-HTML sadržaj; download/print su u lib sloju
 * (lib/diagnostics/exportReport.ts). Determinističke funkcije — lako testabilne.
 */
import type { DiagModuleResult, DiagReportDTO } from "@/types/diagnostics";

export function fmtDiagDate(iso: string): string {
  try {
...
export function diagStateMark(state: DiagModuleResult["state"]): string {
  switch (state) {
    case "ok":
      return "OK";
    case "fail":
...
/** Bezbedno ime fajla iz oznake (za download). */
export function diagFilename(label: string): string {
  return (
    label
      .toLowerCase()
      .replace(/[^a-z0-9]+/gi, "-")
...
export function reportsToText(label: string, reports: DiagReportDTO[]): string {
  ...
  lines.push(`Generisano: ${fmtDiagDate(new Date().toISOString())}`);
  lines.push("=".repeat(60));

  reports.forEach((r, i) => {
    lines.push("");
    lines.push(`[${i + 1}] ${fmtDiagDate(r.createdAt)}`);
    ...
    if (r.results && r.results.length > 0) {
      ...
      for (const m of r.results) {
        ...
        lines.push(`    - [${diagStateMark(m.state)}] ${m.name}${ms}`);
...
export function reportsToMarkdown(
  ...
  md.push(`- **Generisano:** ${fmtDiagDate(new Date().toISOString())}`);
  md.push("");

  reports.forEach((r, i) => {
    md.push(`## ${i + 1}. ${fmtDiagDate(r.createdAt)}`);
    md.push("");
      ...
      for (const m of r.results) {
        ...
        const data =
          m.data && Object.keys(m.data).length > 0
            ...
            : "";
        md.push(
          `| ${m.name} | ${diagStateMark(m.state)} | ${m.ms != null ? `${m.ms} ms` : "—"} | ${detail}${data} |`,
        );
      }
    } else {
...
// ─── PDF: samostalan HTML dokument (štampa se u lib sloju) ────────────────────
export function reportsToPdfHtml(
  label: string,
  reports: DiagReportDTO[],
): string {
  const rows = reports
    .map((r, i) => {
        ...
        r.results && r.results.length > 0
          ? r.results
              ...
              <tr>
                ...
                <td class="s-${m.state}">${diagStateMark(m.state)}</td>
      ...
      return `
        <section>
          <h2>${i + 1}. ${escapeHtml(fmtDiagDate(r.createdAt))}</h2>
          <p class="meta">
  ...
  return `<!doctype html><html lang="sr"><head><meta charset="utf-8">
    ...
    <p class="sub">Izveštaja: ${reports.length} · Generisano: ${escapeHtml(fmtDiagDate(new Date().toISOString()))}</p>
    ${rows}

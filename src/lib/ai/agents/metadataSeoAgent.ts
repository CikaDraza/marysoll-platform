...
function buildMetadataSummary(seo: SeoData, seoContext?: SeoCmsContext) {
...
export async function analyzeMetadataSeo(
  ...
  const content = buildMetadataSummary(seo, seoContext);
  ...
  try {
    return JSON.parse(aiContent) as MetadataSeoAnalysisOutput;
  } catch (e) {

...
export function Theme4FAQSection({
  headline,
  subheadline,
  items,
  supportText,
  ...
  return (
    <div className="bg-[#4C2D4A] py-24 sm:py-32 px-3 lg:px-0">
      <div className="mx-auto grid max-w-7xl gap-20 xl:grid-cols-4">
        <div className="col-span-2">
          <h2 className="text-5xl font-semibold tracking-tight text-pretty text-white lg:text-6xl">
            {headline || "Često postavljana pitanja"}
          </h2>
          {subheadline && (
            <p className="mt-3 text-base text-gray-200">{subheadline}</p>
          )}
          ...
          {faqs.map((faq, i) => (
            <li key={i}>
              <div className="flex items-center gap-x-6">
                <div>
                  <h3 className="text-base/7 font-semibold tracking-tight text-(--secondary-color)">
                    {faq.question}
                  </h3>

    ...
    <section id="gallery" className="py-20 lg:py-32 bg-[var(--background)]">
      <div className="max-w-7xl mx-auto px-6 lg:px-12">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 lg:gap-6">
          {images.map((item, idx) => (
            <div key={idx} className="group relative aspect-square bg-white overflow-hidden cursor-pointer">
              {item.src ? (
                <img
                  src={item.src}
                ...
                <div className="w-full h-full bg-gradient-to-br from-[#E8D5C4] via-[#D4B5A0] to-[#C4A595] flex items-center justify-center">
                  <span className="text-white/80 text-xs font-mono text-center px-4">[{item.title}]</span>
                </div>
              )}
              <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                <div className="text-center text-white space-y-2">
                  {item.title && <p className="text-sm font-light tracking-wide">{item.title}</p>}

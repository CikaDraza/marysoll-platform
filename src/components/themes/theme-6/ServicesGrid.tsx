      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-12">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8 lg:gap-10">
          {displayServices.map((service) => (
            <a key={String(service._id)} href={`${base}/usluge`} className="group">
              <div className="space-y-6">
                <div className="aspect-[3/4] bg-white overflow-hidden">
                  <div className="w-full h-full bg-gradient-to-br from-[#E8D5C4] to-[#D4B5A0] flex items-center justify-center">
                    <span className="text-[var(--muted)] text-sm font-mono">[{service.name}]</span>
                  </div>
                </div>
                <div className="space-y-2">
                  ...
                  {service.description && (
                    <p className="text-sm font-light text-[var(--muted)] leading-relaxed">{service.description}</p>
                  )}
                </div>
              </div>
            </a>

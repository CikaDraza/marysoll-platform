  ...
  return (
    <section className="bg-gray-950 py-24 px-6">
      <div className="max-w-5xl mx-auto">
        ...
        <h2 className="text-3xl font-black text-white text-center mb-16">Zadovoljni klijenti</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {testimonials.slice(0, 3).map(t => (
            <div key={t._id} className="bg-gray-900 border border-gray-800 rounded-2xl p-6 hover:border-yellow-500/20 transition">
              <div className="flex gap-1 mb-4">
                {Array.from({length: 5}).map((_, i) => (
                  <StarIcon key={i} className={`w-4 h-4 ${i < t.rating ? "text-yellow-400 fill-yellow-400" : "text-gray-700"}`} />
                ))}
              </div>
              <p className="text-gray-400 text-sm italic leading-relaxed">&ldquo;{t.comment}&rdquo;</p>
              <div className="flex items-center gap-3 mt-5 pt-4 border-t border-gray-800">
                ...
                <span className="text-gray-300 text-sm font-semibold">{t.clientName}</span>
              </div>
            </div>
          ))}
        </div>

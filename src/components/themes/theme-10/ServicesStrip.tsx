...
export function Theme10ServicesStrip() {
  return (
    <section
      ...
      aria-label="Usluge"
      className="border-b border-ash-ink/8 bg-ash-paper py-[clamp(44px,5vw,72px)]"
    >
      ...
      <div className="grid grid-cols-2 gap-0 min-[900px]:grid-cols-[repeat(auto-fit,minmax(220px,1fr))]">
        {PILLARS.map((p, i) => (
          <div
            key={p.n}
            className={`flex gap-5 border-ash-ink/10 px-[clamp(16px,2.4vw,34px)] py-[clamp(14px,2vw,26px)] max-[899px]:border-t max-[899px]:[&:nth-child(-n+2)]:border-t-0 ${

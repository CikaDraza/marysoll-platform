...
interface Props {
  block: Extract<LayoutBlock, { type: "PricingBlock" }>;
}
  ...
  return (
    ...
    >
      ...
      <div className="mx-auto max-w-7xl text-center">
        <h2 className="text-base/7! font-semibold text-(--secondary-color)">
          Plan
        </h2>
      ...
      <p className="mx-auto mt-6 max-w-2xl text-center text-lg font-medium text-pretty text-gray-600 sm:text-xl/8">
        ...
        angažovanje publike, stvaranje lojalnosti kupaca i povećanje prodaje.
      </p>
      <div className="mx-auto mt-16 grid max-w-lg grid-cols-1 items-center gap-y-6 sm:mt-20 sm:gap-y-0 lg:max-w-4xl lg:grid-cols-1">
        {plans.map((plan) => (
          ...
          >
            <h3
              id={plan.id}
              className={"text-(--secondary-color) text-base/7 font-semibold"}
            >
              {plan.name}
            </h3>
            ...
            <Link
              ...
              className={
                "bg-(--secondary-color) text-white shadow-xs hover:bg-(--secondary-color)/90 focus-visible:outline-(--secondary-color) mt-8 block rounded-md px-3.5 py-2.5 text-center text-sm font-semibold focus-visible:outline-2 focus-visible:outline-offset-2 sm:mt-10"
              }
            >
              {plan.ctaLabel}
            </Link>

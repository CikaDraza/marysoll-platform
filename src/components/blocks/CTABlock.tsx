...
interface Props {
  block: Extract<LayoutBlockType, { type: "CTABlock" }>;
}

export function CTABlock({ block }: Props) {
  const { id, ctaLabel, href, align, size, visibility, variant, className } =
    block;
  ...
  return (
    <Link id={id} href={href || "/"} className={styles + className}>
      {ctaLabel}
    </Link>
  );
}

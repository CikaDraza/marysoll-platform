  ...
  return (
    <>
      ...
      {(block.images ?? []).map((image, index) => <div key={`${image.src}-${index}`} className="space-y-2 rounded border p-2">
        ...
        <ImageMediaField label={`Hero slika ${index + 1}`} image={image} adapter={mediaAdapter} onChange={(next) => onChange({ ...block, images: next ? (block.images ?? []).map((current, itemIndex) => itemIndex === index ? next : current) : (block.images ?? []).filter((_, itemIndex) => itemIndex !== index) })} />
      </div>)}

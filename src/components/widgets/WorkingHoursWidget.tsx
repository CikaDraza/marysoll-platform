...
export function WorkingHoursWidget({ profile }: { profile: SalonProfileData }) {
  ...
  return (
    <div className={`${card}`}>
      <h3 className="text-[11px] font-bold text-zinc-400 dark:text-gray-300 uppercase tracking-widest mb-3">
        🕐 Radno vreme
      </h3>
      <ul className="space-y-1.5">
        {hours.map(({ day, hours: h, isOpen }) => (
          <li key={day} className="flex items-center justify-between">
            <span
              className={`text-xs font-medium w-28 ${isOpen ? "text-zinc-500 dark:text-gray-300" : "text-gray-400"}`}
            >
              {day}
            </span>
            <span
              className={`text-xs font-semibold px-1.5 py-0.5 rounded-full text-zinc-800 dark:text-gray-300 ${
                isOpen
